...
async function checkPlaylists(refresh = false) {
  ...
  message('#connect-message', 'Loading playlists… This can take a moment after the host wakes up.');
  $('#install-output').hidden = true;
  $('#playlist-status').hidden = true;
  try {
    const response = await fetch(refresh ? '/api/refresh' : '/api/status', {
      method:refresh ? 'POST' : 'GET', headers:{Authorization:`Bearer ${accessKey}`}, signal:AbortSignal.timeout(180000)
    });
    ...
    for (const source of data.sources) {
      ...
      if (source.state === 'ready') {
        const s = source.stats; total += s.accepted;
      ...
      } else { failed++; detail.textContent = source.error; row.append(content); }
  ...
  } catch (error) { message('#connect-message', error.name === 'TimeoutError' ? 'The request timed out. Open the host, wait for it to wake, and try again.' : error.message, true); }
  finally { button.disabled = false; $('#refresh').disabled = false; }
}
